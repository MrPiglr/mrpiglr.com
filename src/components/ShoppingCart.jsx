...
const ShoppingCart = ({ isCartOpen, setIsCartOpen }) => {
  ...
  const handleCheckout = useCallback(async () => {
    ...
    try {
      // Filter out service items (fake variants) to prevent API errors
      const validItems = cartItems.filter(item => !item.variant.id.toString().startsWith('service-variant-'));

      if (validItems.length === 0 && cartItems.length > 0) {
         toast({
            title: 'Cannot Checkout',
            description: 'Services cannot be purchased directly. Please inquire on the Services page.',
            variant: 'destructive',
         });
         return;
      }
